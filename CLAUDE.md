...
- Event classification runs against a three-tier taxonomy in `services/ingest/taxonomy.py` (August 2026), but **the end user only ever sees one simple category**. The LLM is shown only the ~120 specific leaf events ("ceo_departure", "covenant_violation") and answers with 1-3 of them; `to_labels()` collapses those onto the taxonomy's 8 primary buckets + "Other" — that 9-label list (`taxonomy.CATEGORIES`) is the whole product surface, mirrored in `services/api/app/routes/events.py` (`GET /events/types` backs the feed's category filter) and in sensybull-web's toolbar fallback. Keep those in sync. Adding a leaf is cheap and changes nothing the user sees; adding a primary bucket adds a filter chip, so that's a product decision. The chosen leaves ride along on `briefing.taxonomy` (+ `taxonomy_version`) for analytics — never render them. Facts-only briefings make no leaf claim: the SEC item number maps straight to a category via `taxonomy.ITEM_CATEGORIES` (7.01/8.01 deliberately unmapped → "Other"). Press releases classify against the same taxonomy, so wires and filings land in the same categories; a release no leaf fits is dropped. Events written before the taxonomy keep their old 12 labels forever — `taxonomy.LEGACY_LABELS` folds them into the current ones at read time (mirrored in `routes/events.py` `LEGACY_EVENT_TYPES` and sensybull-web `src/hooks/use-events.ts`).
- The taxonomy leaf list lives inside every briefing prompt (~1.2K tokens), which is why `briefing._TOTAL_TEXT_CAP` dropped from 16K to 10K chars and `materiality._BODY_TEXT_CAP` from 12K to 9K — the whole request has to fit under the 8K TPM ceiling. Check that arithmetic before growing either the prompt or the caps.
