...
- Ingest pipeline: SEC EDGAR polling → Groq LLM briefing → Redis → API. A second poller in the same worker (`services/ingest/press_release/`, flag `PR_INGEST_ENABLED`, off by default) ingests newswire press releases (GlobeNewswire/PR Newswire/Accesswire RSS; Business Wire via `PR_FEED_URLS_BUSINESSWIRE`): promo prefilter + law-firm blocklist + issuer-vs-ticker verification + LLM first-party/materiality gate; publishes `signal_type="PR"` events on the same Redis contract (synthetic `edgar_id="pr:<wire>:<guid>"`, no facts_only fallback — unclassifiable releases are dropped)
